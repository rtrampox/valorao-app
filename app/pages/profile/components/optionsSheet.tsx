...
export function OptionsSheet({ profileData, setIsLoggingOut, isLoggingOut, bottomSheetRef }:
    { profileData: PlayerProfile | null, setIsLoggingOut: (isLoggingOut: boolean) => void, isLoggingOut: boolean, bottomSheetRef: React.RefObject<BottomSheet> }) {
    ...
    async function enableNotifications() {
        ...
        if (perm.authorizationStatus === AuthorizationStatus.DENIED) return await AsyncStorage.setItem('user/permissions/notification', 'false')
        if (perm.authorizationStatus === AuthorizationStatus.AUTHORIZED) return await AsyncStorage.setItem('user/permissions/notification', 'true')
    }
    ...
    return (
        <BottomSheet
            ref={bottomSheetRef}
            snapPoints={profileData?.playerName === "OPULENCE#RENY" ? [55, 380] : [55, 280]}
                ...
                <View className="w-full gap-4 px-5">
                    ...
                    {profileData?.playerName === "OPULENCE#RENY" &&
                        <View className="text-center w-full gap-2">
                            <Text className="text-white text-center">Para desenvolvedores:</Text>
                            <Button
                    ...
                    <View>
                        <Text className="text-white text-sm text-center -m-3">O aplicativo deve ser reiniciado para fazer efeito.</Text>
                    </View>
                    <View className="h-[1px] bg-muted-foreground" />
                    <Button
                        onPress={() => logout()}
