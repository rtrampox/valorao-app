...
export async function getUserLastMatches(): Promise<
...
> {
	...
	const getMatchDetails: Promise<MatchDetailsResponse>[] =
		getMatches.data.History.map(async (item) => {
			const matchDetails = await axios.get<MatchDetailsResponse>(
				`https://pd.na.a.pvp.net/match-details/v1/matches/${item.MatchID}`,
				{ headers: headers },
			);
...
export async function getProfileFromCache(): Promise<
	MatchDetailsResponse[] | null
> {
	try {
