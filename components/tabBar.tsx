...
const TabBar = ({ state, descriptors, navigation }: BottomTabBarProps) => {
	...
	const item: { [key: string]: (props: LucideProps) => JSX.Element } = {
		...
		"Mercado Noturno": (props: LucideProps) => (
			<CircleDollarSignIcon {...props} />
		),
	};
	const colors = {
		primary: "#0a0a0a",
	...
	return (
		<View
			className={`flex-row justify-between gap-1 bg-neutral-800 mx-2 items-center py-2 px-2 absolute bottom-2 ${pageTitle === 2 ? "rounded-b-3xl" : "rounded-full shadow-black shadow-xl"}`}
		>
			{state.routes.map((route, index) => {
				...
				return (
					...
					>
						{Object.keys(item).includes(label.toString()) ? (
							item[label.toString()]({
								size: 14,
						...
						<Text
							style={{ color: isFocused ? colors.textSelected : colors.text }}
						>
							{label.toString()}
						</Text>
					</TouchableOpacity>
